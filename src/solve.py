...
def solve_milp(type, gama, gap):
    ...
    v_m_t = {(m, t): model.binary_var(name=f'v_{m}_{t}')
             for m in range(n_m) for t in range(T)}
    v_mp_t = {(mp, t): model.binary_var(name=f'v_{mp}_{t}')
              for mp in range(n_mp) for t in range(T)}

    # 控制生产线运行状态-PV cell processing 产线
    v_c_t = {(c, t): model.binary_var(name=f'v_{c}_{t}')
             for c in range(n_c) for t in range(T)}
    v_cp_t = {(cp, t): model.binary_var(name=f'v_{cp}_{t}')
    ...
    P_MTG_i_t = {(i, t): model.continuous_var(name=f'P_MTG_{i}_{t}')
                 for i in range(n_G) for t in range(T)}
    # t 时第 i 个 MTG 的维护需要的能量
    u_MTG_i_t = {(i, t): 0 for i in range(n_G) for t in range(T)}
    # t 时 ESS 充电量
    P_ESS_ch_t = {t: model.continuous_var(
        name=f'P_ESS_ch_{t}') for t in range(T)}
    # t 时 ESS 放电量
    P_ESS_dc_t = {t: model.continuous_var(
        name=f'P_ESS_dc_{t}') for t in range(T)}
    # t 时 PIES 从电网买电量
    P_b_gr_t = {t: model.continuous_var(
        lb=0, ub=P_b_gr_max, name=f'P_b_gr_{t}') for t in range(T)}
    # t 时 PIES 向电网卖电量
    P_s_gr_t = {t: model.continuous_var(
        lb=0, ub=P_s_gr_max, name=f'P_s_gr_{t}') for t in range(T)}
    # t 时转移进的输入负载
    P_TL_in_t = {t: model.continuous_var(
        name=f'P_TL_in_{t}') for t in range(T)}
    # t 时转移出的输出负载
    P_TL_out_t = {t: model.continuous_var(
        name=f'P_TL_out_{t}') for t in range(T)}
    # t 时中断负载
    P_IL_t = {t: model.continuous_var(
        lb=P_IL_t_min, ub=P_IL_t_max, name=f'P_IL_{t}') for t in range(T)}
    # t 时注入建筑 w 的冷空气量
    C_IL_c_w_t = {(w, t): model.continuous_var(name=f'C_IL_c_{w}_{t}')
    ...
    # MTG 操作成本
    C_MTG = model.sum(model.sum(
        a_i[i] * P_MTG_i_t[i, t] + b_i * u_MTG_i_t[i, t] for i in range(n_G)) for t in range(T))
    # =====================计算 ESS 操作成本=====================
    ...
    # ESS 操作成本
    C_ESS = model.sum(
        epsilon_ESS * (P_ESS_ch_t[t] + P_ESS_dc_t[t]) for t in range(T))
    ...
    # =====================计算 DR 补偿成本=====================
    # DR 补偿成本
    C_DR = model.sum(rho_TL * P_TL_in_t[t] + rho_IL * P_IL_t[t] +
                     rho_c * C_IL_c_w_t[w, t] for w in range(W) for t in range(T))
    # =====================计算太阳能削减成本=====================
    # !
